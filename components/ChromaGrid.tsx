...
const ChromaGrid: React.FC<ChromaGridProps> = ({
  ...
  return (
    <div
      ...
      className={`absolute w-full h-full rounded-10 flex flex-wrap justify-center items-start gap-3 ${className}`}
      style={
        {
          '--r': `${radius}px`,
          '--x': '50%',
    ...
    >
      {data.map((c, i) => (
        <article
          ...
          className="group relative flex flex-col w-[800px] rounded-[10px] overflow-hidden border-2 border-transparent transition-colors duration-300 cursor-pointer"
          style={
            {
        ...
        >
          ...
          <div className="relative z-10 flex-1 p-[10px] box-border">
            <img src={c.image} alt={c.title} loading="lazy" className="w-full h-full object-cover rounded-lg" />
          </div>
